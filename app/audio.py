...
def _to_mono_float32(arr) -> np.ndarray:
    # Tensors are handled through __array__; numpy arrays pass through.
    arr = np.asarray(arr)
    if arr.ndim == 2:
...
def encode(samples, sample_rate: int, fmt: str) -> Tuple[bytes, str]:
    fmt = fmt.lower()
